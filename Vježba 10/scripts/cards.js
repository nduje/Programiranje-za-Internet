...
async function handleHeartIconClick(e){
    ...
    let parentCard = heartIcon.closest(".card");

    let cardId = parentCard.getAttribute("data-card-id");
    let isCurrentlyLiked = heartIcon.classList.contains("fa-heart");

    try {
        let serverResponse = await fetch(
            `API.php?action=toggleCardLike&id=${cardId}&like=${isCurrentlyLiked ? 0 : 1}`
        );
...
async function handleDeleteCardClick(e){
     ...
     let parentCard = deleteCardIcon.parentElement;
     let cardTitle = parentCard.querySelector("h3");
     ...
     if (confirm("Izbrisati karticu: " + cardTitle.textContent + "?")) {
        try {
            let serverResponse = await fetch(
                `API.php?action=deleteCard&id=${cardId}`
            );
...
addCardButton.addEventListener("click", async function(e) {
    ...
    cardElement.children[0].setAttribute("data-card-id", responseData.id);
    ...
    let newDeleteButton = cardElement.querySelector(".delete-button");
